...
@_Jac.make_walker(on_entry=[_Jac.DSFunc('return_message', _Jac.RootType)], on_exit=[])
@__jac_dataclass__(eq=False)
class add_7(_Jac.Walker):
    number: int

    def return_message(self, _jac_here_: _Jac.RootType) -> None:
        _Jac.report({'response': self.number + 7})

@_Jac.make_walker(on_entry=[_Jac.DSFunc('return_message', _Jac.RootType)], on_exit=[])
@__jac_dataclass__(eq=False)
class add_7_with_body(_Jac.Walker):
    name: str
    number: int

    def return_message(self, _jac_here_: _Jac.RootType) -> None:
        _Jac.report({'response': 'Hello, ' + self.name + '! Your number plus 7 is: ' + str(self.number + 7)})